...
def train(dataset, batch_size, backbone_name, config):
    summary_writer = SummaryWriter(os.path.join(config.output_dir, 'summaries_r2'))
    epochs = config.epochs
    dataloader = DataLoader(dataset,batch_size,num_workers=8,shuffle=False,collate_fn = dataset.padding_collate_fn) 
    ...
    for epoch in range(epochs):
        ...
        for it, (image_id, image_batch, scale, bboxes_batch, labels_batch) in enumerate(dataloader):
            if(step == 720000):
                torch.save(model.state_dict(),'outputs/model_resnet101_r2.pth')
                stop = 1
                break
            else:
...
if __name__ == '__main__':
    config = utils.Params('config_resnet101_r2.json')
