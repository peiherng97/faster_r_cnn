...
def train(dataset, batch_size, backbone_name, config):
    summary_writer = SummaryWriter(os.path.join(config.output_dir, 'summaries'))
    ...
    for epoch in range(epochs):
        for it, (image_id, image_batch, scale, bboxes_batch, labels_batch) in enumerate(dataloader):
                ...
                if(it%500 == 0):
                  torch.save(model.state_dict(),'outputs'+'/model_vgg16.pth')
                loss.backward()
                optimizer.step()
                step += 1

if __name__ == '__main__':
    config = utils.Params('config_vgg16.json')
